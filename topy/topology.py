...
class Topology:
    ...
    def set_top_params(self):
        ...
        logger.info('Element type (ELEM_K) =', self.topydict['ELEM_TYPE'])
        logger.info('Filter radius (FILT_RAD) =', self.filtrad)
    ...
    def fea(self):
        ...
        if self.dofpn < 3 and self.nelz == 0: #  Direct solver
            ...
            if info < 0:
                logger.info('PySparse error: Type:', info,', at', numitr, \
                    'iterations.')
                raise Exception('Solution for FEA did not converge.')
            else:
                logger.info('ToPy: Solution for FEA converged after', numitr, \
                    'iterations.')
